 ...
 * </PRE>
 */
public class PShapeSVGExtended extends PShapeExtended {
  XML element;
...
//      fillGradientPaint = parent.fillGradientPaint;
  ...
  protected void parseChildren(XML graphics) {
    ...
    children = new PShapeExtended[elements.length];
    childCount = 0;

    for (XML elem : elements) {
      PShapeExtended kid = parseChild(elem);
      if (kid != null) addChild(kid);
    }
    children = (PShapeExtended[]) PApplet.subset(children, 0, childCount);
  }
   ...
   */
  protected PShapeExtended parseChild(XML elem) {
...
//  }
   ...
   * This assumes 96dpi, which implies, as given in the
   * <A HREF="http://www.w3.org/TR/SVG/coords.html#Units">units</A> spec:
   * <UL>
   * <LI>"1pt" equals "1.25px" (and therefore 1.25 user units)
   * <LI>"1pc" equals "15px" (and therefore 15 user units)
   * <LI>"1mm" would be "1.0" (1.0 user units if it is mm)
   * <LI>"1cm" equals "37.79527px" (and therefore37.79527 user units)
   * <LI>"1in" equals "96px" (and therefore 96 user units)
   * </UL>
   * @param relativeTo (float) Used for %. When relative to viewbox, should
   *    be svgWidth for horizontal dimentions, svgHeight for vertical, and
   *    svgXYSize for anything else.
  ...
  static protected float parseUnitSize(String text, float relativeTo) {
    ...
    if (text.endsWith("pt")) {
      ...
      return PApplet.parseFloat(text.substring(0, len)) * 1f;
    } else if (text.endsWith("cm")) {
      return PApplet.parseFloat(text.substring(0, len)) *  37.79527f;
    } else if (text.endsWith("in")) {
      return PApplet.parseFloat(text.substring(0, len)) * 96;
    } else if (text.endsWith("px")) {
      return PApplet.parseFloat(text.substring(0, len));
    } else if (text.endsWith("%")) {
      return relativeTo * parseFloatOrPercent(text);
...
//    int horizAdvX;     // no dflt?
   ...
   */
  @Override
  public PShapeExtended getChild(String name) {
    PShapeExtended found = (PShapeExtended)super.getChild(name);
    if (found == null) {
      // Otherwise try with underscores instead of spaces
      // (this is how Illustrator handles spaces in the layer names).
      found = (PShapeExtended) super.getChild(name.replace(' ', '_'));
    }
